<?php
    ...
    $pacientacept_id_del = limpiar_cadena($_GET['pacientacept_id_del']);
    ...
    $check_paciente=$check_paciente->query("SELECT paciente_id FROM paciente_aceptado WHERE paciente_id='$pacientacept_id_del'");
    
    if ($check_paciente->rowCount() == 1) {
        $eliminar_paciente = conexion();
        $eliminar_paciente = $eliminar_paciente->prepare("DELETE FROM paciente_aceptado WHERE paciente_id=:id");
    
        $eliminar_paciente->execute([":id" => $pacientacept_id_del]);
