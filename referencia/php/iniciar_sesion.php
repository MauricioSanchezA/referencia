<?php
session_start();  // siempre que esto esté al inicio del script
...
if($check_user->rowCount() == 1){
    ...
    if($check_user['usuario_usuario'] == $usuario && password_verify($clave, $check_user['usuario_clave'])){
        ...
        $_SESSION['role'] = $check_user['role'];  // Aquí guardo el rol

        // Redireccionar al usuario dependiendo de su rol
        if($_SESSION['role'] == 'Administrador'){
            ...
            // Si no es administrador, redirigir a un área específica no visualiza "usuarios"
            if(headers_sent()){
                echo "<script> window.location.href='index.php?vista=home'; </script>";
            } else {
                header("Location: index.php?vista=home");
